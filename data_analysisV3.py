...
words = document.map(lambda w:"/".join(jieba.cut(w, cut_all=True))).map(lambda line: line.split("/")) 	# V3

"""
text=words.flatMap(lambda w:w)
wordCounts = text.map(lambda word: (word, 1)).reduceByKey(lambda a, b: a+b).sortBy(lambda x: x[1], ascending=False)
wordCounts.take(100)
"""
stopwords = set([""," ","，","的","。","了", "！","是","就","吧","?","…","、","～","~","很","都","也","还"])
